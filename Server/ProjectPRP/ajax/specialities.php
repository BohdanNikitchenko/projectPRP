...
if(!$con){
    die("Немає зв'язку із сервером".mysqli_error($con));
}

mysqli_set_charset($con, "utf-8");
$sql = "SELECT * FROM specialty_in_universities WHERE Id_Specialty = 024";
$res = mysqli_query($con, $sql);
while($row = mysqli_fetch_array($res)){
    $id_spec = $row['Id_specialty'];
    $name_spec = $row['Name_Specialty'];
    //$name_un = $row['Name_Universities'];
